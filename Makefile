.PHONY: help install worker-install setup dev desktop worker restart stop clean test test-ts test-py lint format build
...
lint: ## Run all linters
	...
	cd $(WORKER_DIR) && uv run ruff check .

format: ## Format all code
	cd $(DESKTOP_DIR) && npx prettier --write 'src/**/*.{ts,tsx,css}'
	cd $(WORKER_DIR) && uv run ruff format .
