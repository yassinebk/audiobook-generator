...
@dataclass
class R:
    engine: str; device: str; test: str
    text_chars: int; text_words: int
    load_t: float; synth_t: float; total_t: float
    audio_s: float; audio_b: int; rtf: float
    errors: list = field(default_factory=list)
    def d(self): return {
        "engine":self.engine,"device":self.device,"test":self.test,
        "text_chars":self.text_chars,"text_words":self.text_words,
        "load_t":round(self.load_t,3),"synth_t":round(self.synth_t,3),
        "total_t":round(self.total_t,3),"audio_s":round(self.audio_s,3),
        "audio_b":self.audio_b,"rtf":round(self.rtf,3),"errors":self.errors}
...
def parler(text, device, test_name):
    ...
    try:
        ...
        if a.dtype.name=="float16": a=a.astype("float32")
...
def kokoro(text, device, test_name):
    ...
    try:
        ...
        for r in gen:
            s=r.audio
            if torch.is_tensor(s): s=s.cpu().numpy().squeeze()
            if s.ndim>=1 and len(s)>0: segs.append(s)
